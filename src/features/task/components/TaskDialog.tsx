...
function TaskDialogContent({
	...
	const { closeDialog } = useDialog('task');

	// Use Suspense query for task - will suspend if taskId is provided
	const [task] = taskId
		? api.task.getById.useSuspenseQuery({ id: taskId })
		: [null];
	...
	return (
		<Dialog open={isDialogOpen} onOpenChange={closeDialog}>
			<DialogContent className="max-h-[90vh] max-w-7xl overflow-y-auto">
				...
				{taskId ? (
					<>
						{/* Render TaskComments outside Suspense to start fetching immediately */}
						{isDialogOpen && taskId && (
							<div
