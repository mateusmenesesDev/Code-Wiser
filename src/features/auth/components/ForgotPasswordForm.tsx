...
import { forgotPasswordSchema, resetPasswordSchema } from "~/features/schemas/auth.schema";
    ...
    return (
      <form onSubmit={handleSubmitCode(onVerify)} className="space-y-4">
        ...
        <div className="grid gap-2">
          ...
          <InputOTP id="code" maxLength={6} value={code} onChange={(value) => setValue("code", value)} >
            <InputOTPGroup>
  ...
  return (
    <form className="space-y-6" onSubmit={handleSubmit(onSubmit)}>
      ...
      <Button className="w-full" disabled={isSubmitting || timeToResend > 0}>
        {isSubmitting ? "Sending..." : timeToResend > 0 ? `Resend in ${timeToResend}s` : "Send Reset Link"}
      </Button>
    </form>
  );
}
