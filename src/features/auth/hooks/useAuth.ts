...
import type { resetPasswordSchema, signInSchema, signUpSchema, verifyEmailSchema } from "~/features/schemas/auth.schema";
import { api } from "~/trpc/react";
import { authErrors } from "../authErrors";

export const useAuth = () => {
	const { signIn, isLoaded, setActive } = useSignIn();
	const { signUp, isLoaded: isLoadedSignUp, setActive: setActiveSignUp } = useSignUp();
	...
	const verifyEmail = async (data: z.infer<typeof verifyEmailSchema>) => {
		...
		try {
			const { status, emailAddress, createdSessionId, createdUserId } = await signUp.attemptEmailAddressVerification({
				code: data.code,
			});
			...
			if (status === "complete") {
				...
				await createUserMutation.mutateAsync({ email: emailAddress, id: createdUserId });
				return;
			}
			setError("Sign in failed. Please try again.");
			return;
	...
	return { signInWithEmail, signInWithGoogle, error, signUpWithEmail, verifyEmail, isVerifying, forgotPassword, resetPassword };
}
