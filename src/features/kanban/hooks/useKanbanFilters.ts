...
export const useKanbanFilters = () => {
	...
	const sprint =
		kanbanFilters.sprint === 'undefined' || kanbanFilters.sprint === ''
			? undefined
			: kanbanFilters.sprint;
	const priority = kanbanFilters.priority; // Enum, can't be "undefined" string
	const assignee =
		kanbanFilters.assignee === 'undefined' || kanbanFilters.assignee === ''
			? undefined
			: kanbanFilters.assignee;

	const hasActiveFilters = useMemo(() => {
		return (
			sprint !== undefined || priority !== undefined || assignee !== undefined
		);
	...
	const filterTasks = (tasks: KanbanDataOutput | undefined) => {
		...
		return tasks.filter((task) => {
			if (sprint && sprint !== 'all' && task.sprint?.id !== sprint)
				return false;
			if (priority && task.priority !== priority) return false;
			if (assignee && assignee !== 'all' && task.assignee?.id !== assignee)
				return false;
			return true;
		});
	};
